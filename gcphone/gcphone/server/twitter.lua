  ...
  getUser(username, password, function (user)
    ...
    MySQL.Async.insert("INSERT INTO twitter_tweets (`authorId`, `message`, `image`, `realUser`) VALUES(@authorId, @message, @image, @realUser);", {
      ...
      MySQL.Async.fetchAll('SELECT * from twitter_tweets WHERE id = @id', {
        ...
        TriggerClientEvent('gcphone:233223twitter_newTweets', -1, tweet)
        TriggerEvent('gcphone:233223twitter_newTweets', tweet)
      end)
    end)
  end)
end
        ...
        if (row[1] == nil) then
          MySQL.Async.insert('INSERT INTO twitter_likes (`authorId`, `tweetId`) VALUES(@authorId, @tweetId)', {
            ...
            MySQL.Async.execute('UPDATE `twitter_tweets` SET `likes`= likes + 1 WHERE id = @id', {
              ...
              TriggerClientEvent('gcphone:233223twitter_updateTweetLikes', -1, tweet.id, tweet.likes + 1)
              TriggerClientEvent('gcphone:233223twitter_setTweetLikes', sourcePlayer, tweet.id, true)
              TriggerEvent('gcphone:233223twitter_updateTweetLikes', tweet.id, tweet.likes + 1)
            end)    
          end)
        else
          MySQL.Async.execute('DELETE FROM twitter_likes WHERE id = @id', {
            ...
            MySQL.Async.execute('UPDATE `twitter_tweets` SET `likes`= likes - 1 WHERE id = @id', {
              ...
              TriggerClientEvent('gcphone:233223twitter_updateTweetLikes', -1, tweet.id, tweet.likes - 1)
              TriggerClientEvent('gcphone:233223twitter_setTweetLikes', sourcePlayer, tweet.id, false)
              TriggerEvent('gcphone:233223twitter_updateTweetLikes', tweet.id, tweet.likes - 1)
            end)
          end)
        end
...
function TwitterShowError (sourcePlayer, title, message, image)
  TriggerClientEvent('gcphone:233223twitter_showError', sourcePlayer, message, image)
end
function TwitterShowSuccess (sourcePlayer, title, message, image)
  TriggerClientEvent('gcphone:233223twitter_showSuccess', sourcePlayer, title, message, image)
end

RegisterServerEvent('gcphone:233223twitter_login')
AddEventHandler('gcphone:233223twitter_login',function(username,password)
	local sourcePlayer = tonumber(source)
	getUser(username,password,function(user)
		if user ~= nil then
			TriggerClientEvent('gcphone:233223twitter_setAccount',sourcePlayer,username,password,user.authorIcon)
		end
	end)
end)


RegisterServerEvent('gcphone:233223twitter_changePassword')
AddEventHandler('gcphone:233223twitter_changePassword', function(username, password, newPassword)
  local sourcePlayer = tonumber(source)
  getUser(username, password, function (user)
    if user == nil then
      ...
      MySQL.Async.execute("UPDATE `twitter_accounts` SET `password`= @newPassword WHERE twitter_accounts.username = @username AND twitter_accounts.password = @password", {
        ...
        if (result == 1) then
          TriggerClientEvent('gcphone:233223twitter_setAccount', sourcePlayer, username, newPassword, user.authorIcon)
...
RegisterServerEvent('gcphone:233223twitter_createAccount')
AddEventHandler('gcphone:233223twitter_createAccount', function(username, password, avatarUrl)
  local sourcePlayer = tonumber(source)
  TwitterCreateAccount(username, password, avatarUrl, function (id)
    if (id ~= 0) then
      TriggerClientEvent('gcphone:233223twitter_setAccount', sourcePlayer, username, password, avatarUrl)
...
RegisterServerEvent('gcphone:233223twitter_getTweets')
AddEventHandler('gcphone:233223twitter_getTweets',function(username,password)
	local sourcePlayer = tonumber(source)
	if username ~= nil and username ~= "" and password ~= nil and password ~= "" then
		getUser(username,password,function(user)
			local accountId = user and user.id
			TwitterGetTweets(accountId,function(tweets)
				TriggerClientEvent('gcphone:233223twitter_getTweets',sourcePlayer,tweets)
			end)
		end)
	else
...
RegisterServerEvent('gcphone:233223twitter_getFavoriteTweets')
AddEventHandler('gcphone:233223twitter_getFavoriteTweets', function(username, password)
  local sourcePlayer = tonumber(source)
  if username ~= nil and username ~= "" and password ~= nil and password ~= "" then
    getUser(username, password, function (user)
      local accountId = user and user.id
      TwitterGetFavotireTweets(accountId, function (tweets)
        TriggerClientEvent('gcphone:233223twitter_getFavoriteTweets', sourcePlayer, tweets)
      end)
    end)
  else
...
RegisterServerEvent('gcphone:233223twitter_getUserTweets')
AddEventHandler('gcphone:233223twitter_getUserTweets', function(username, password)
  local sourcePlayer = tonumber(source)
  if username ~= nil and username ~= "" and password ~= nil and password ~= "" then
    getUser(username, password, function (user)
      local accountId = user and user.id
      TwitterUsersTweets(accountId, function (tweets)
        TriggerClientEvent('gcphone:233223twitter_getUserTweets', sourcePlayer, tweets)
      end)
    end)
  else
...
RegisterServerEvent('gcphone:233223twitter_postTweets')
AddEventHandler('gcphone:233223twitter_postTweets',function(username,password,message,image)
	local sourcePlayer = tonumber(source)
	local srcIdentifier = getPlayerID(source)
	TwitterPostTweet(username,password,message,image,sourcePlayer,srcIdentifier)
end)

RegisterServerEvent('gcphone:233223twitter_toogleLikeTweet')
AddEventHandler('gcphone:233223twitter_toogleLikeTweet',function(username,password,tweetId)
	local sourcePlayer = tonumber(source)
	TwitterToogleLike(username,password,tweetId,sourcePlayer)
end)

RegisterServerEvent('gcphone:233223twitter_usersDeleteTweet')
AddEventHandler('gcphone:233223twitter_usersDeleteTweet', function(username, password, tweetId)
  local sourcePlayer = tonumber(source)
  TwitteUsersDelete(username, password, tweetId, sourcePlayer)
end)

RegisterServerEvent('gcphone:233223twitter_setAvatarUrl')
AddEventHandler('gcphone:233223twitter_setAvatarUrl', function(username, password, avatarUrl)
  local sourcePlayer = tonumber(source)

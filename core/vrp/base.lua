...
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookjoins = "https://discord.com/api/webhooks/816414411425447936/l4spujrCUJcxNbtCmZfxAQYmCFtAmdy_9sTDId33fSoPVk8tkhbk2458fgaBCuG6uI15"
	...
	if steam then
		if not vRP.isBanned(steam) then
			if vRP.isWhitelisted(steam) then
				...
				deferrals.done("Faca sua whitelist pelo bot e na parte de steam hex coloque: "..steam)
...
AddEventHandler("baseModule:idLoaded",function(source,user_id,model)
	...
	if vRP.rusers[user_id] == nil then
		...
		if model ~= nil then
			TriggerClientEvent("Notify",source,"importante","Seja bem-vindo ao <b>Capão Redondo INFINITY</b>.<br>A cidade foi desenvolvida pensando especialmente em voce, desenvolva sua historia e nao esqueca que deixamos separado para voce um agrado em seu banco.",15000)
			...
			vRP.user_tables[user_id].inventorys["5"] = { item = "dollars", amount = 3000 }
		end

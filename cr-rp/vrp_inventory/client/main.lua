                ...
                for key,value in pairs(AllWeapons) do
                    for keyTwo,valueTwo in pairs(AllWeapons[key]) do
                        if weapon == GetHashKey('weapon_'..keyTwo) then
                            if keyTwo and ammoTotal <= 2 then
                                vSERVER.updateWeaponAmmo("WEAPON_"..keyTwo,0)
                            else
...
-- PUTWEAPONHANDS
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterNetEvent('vrp_inventory:client:takingWeapon')
AddEventHandler('vrp_inventory:client:takingWeapon', function(bool)
     takingWeapon = bool
end)

...
RegisterCommand("derrubar", function(source, args, rawCommand)
    local user_id = vRP.getUserId(source)
    if user_id then
        if vRP.hasPermission(user_id, "Admin") and args[1] and parseInt(args[1]) > 0 then
            local target_id_arg = parseInt(args[1])
            local nplayer = vRP.getUserSource(target_id_arg)

            if nplayer then
                local t_user_id = vRP.getUserId(nplayer)
                local identity = vRP.getUserIdentity(user_id)
				
                TriggerClientEvent("Notify", source, "sucesso", "Você fez o ID <b>" .. t_user_id .. "</b> Capotar.", 2000)
                TriggerClientEvent("capotar", nplayer)
				
                local webhookadmin = "https://discord.com/api/webhooks/1366945440196460606/pSbWVkVEtx_zSbp8K7s49uIg2IyQ4kpHlK62tnbozIjGM4en7yoTHG-6yv9LNzbtWQ3N" -- Substitua pela sua URL
                if webhookadmin then
                    local logData = {
                        embeds = {
                            {
                                title = "REGISTRO DE CAPOTAMENTO:⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀",
                                thumbnail = {
                                    url = "https://discord.com/api/webhooks/..." -- Sua URL do thumbnail (opcional)
                                },
                                fields = {
                                    {
                                        name = "**QUEM FOI CAPOTADO:**",
                                        value = "` " .. (identity.name) .. " " .. (identity.firstname) .. " [" .. user_id .. "] `"
                                    },
                                    {
                                        name = "**QUEM CAPOTOU O INDIVIDUO:**",
                                        value = "` " .. (identity.name) .. " " .. (identity.firstname) .. " [" .. user_id .. "] `"
                                    }
                                },
                                footer = {
                                    text = os.date("\n[Data]: %d/%m/%Y [Hora]: %H:%M:%S"),
                                    icon_url = "https://cdn.discordapp.com/attachments/..." -- Sua URL do ícone do footer (opcional)
                                },
                                color = 0225500
                            }
                        }
                    }
                    print("Dados da log:", json.encode(logData))
                    PerformHttpRequest(webhookadmin, function(err, text, headers)
                    end, 'POST', json.encode(logData), { ['Content-Type'] = 'application/json' })
                else
                    print("URL do webhook não configurada.")
                end
            else
                TriggerClientEvent("Notify", source, "erro", "Jogador com ID " .. target_id_arg .. " não encontrado.", 1000)
            end
        else
            TriggerClientEvent("Notify", source, "erro", "Uso: /derrubar [ID do jogador]", 1000)
        end
    end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
...
-- HASHDOORS
-----------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand("hashdoors",function(source,args,rawCommand)
	local user_id = vRP.getUserId(source)
		if vRP.hasPermission(user_id, "Admin") then
			vCLIENT.hashdoors(source)
		end
	end 
end)
-----------------------------------------------------------------------------------------------------------------------------------------
			...
			if not vRP.hasPermission(user_id,"Police") then
				...
				if distance then
					...
					for k,v in pairs(comAmount) do
						async(function()
							TriggerClientEvent("NotifyPush",v,{ time = os.date("%H:%M:%S - %d/%m/%Y"), text = "Ei esta tendo troca de tiro aqui perto de minha casa!", code = 1, title = "Confronto em andamento", x = x, y = y, z = z, criminal = "Disparos de arma de fogo", rgba = {105,52,136} })
						end)
					end
				end
			end

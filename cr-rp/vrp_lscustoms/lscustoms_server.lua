...
-----------------------------------------------------------------------------------------------------------------------------------------
function vRP.tryPayment(user_id,amount) -- PAGAMENTO COM CARTÃO
    if amount >= 0 then
        local source = vRP.getUserSource(user_id)
        if vRP.getInventoryItemAmount(user_id,"cartaodebito") >= 1 then
            local money = vRP.getBank(user_id)
            if amount >= 0 and money >= amount then
                vRP.setBank(user_id,money-amount)
                TriggerClientEvent("itensNotify",source,{ "REMOVIDO","dollars",vRP.format(amount),"R$" })
                return true
            else
                return false
            end
        else
            if amount >= 0 and vRP.getInventoryItemAmount(user_id,"dollars") >= amount then

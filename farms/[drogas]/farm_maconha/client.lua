...
Citizen.CreateThread(function()
	while true do
        ...
        if not premisa then
            ...
            if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), 2932.3229980469,4624.2119140625,48.723407745361, true) <= 1.0 then	
                skips = 1
                DrawText3Ds(2932.32,4624.21,48.72+0.5,"TRABALHANDO, VÁ PLANTAR.")
                ...
                if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), v.x,v.y,v.z, true) <= 1.5 then
                    ...
                    if (obj == 0 and obj2 == 0)  then
                        if v.state ~= 3 and v.plantado == false then
                            skips = 1
                            DrawText3Ds(v.x,v.y,v.z+0.5,"PRESSIONE ~r~E~w~ PARA PLANTAR UMA MACONHA")
                            if IsControlJustPressed(0, 38) then
...
local closest = {}
local cadeiras = {
	{objectId = -1281587804, x = 1769.8929, y = -1619.67, z = 113.63, h = 120.0}, -- cadeira DROGA   112.52102661133,6360.16796875,32.787044525146
}
...
Citizen.CreateThread(function()
	while true do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), 1769.8996582031,-1619.6729736328,113.63678741455, true) <= 1.5 or GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), 116.44131469727,6362.4877929688,32.785820007324, true) <= 0.8   then
            if closest then

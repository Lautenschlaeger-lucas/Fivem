...
local min_policial = 4
local policial_ptr_permissao  = "policia-ptr.permissao"
-----------------------------------------------------------------------------------------------------------------------------------------
-- WEBHOOK
-----------------------------------------------------------------------------------------------------------------------------------------
local webhookdepartamento = "https://discord.com/api/webhooks/810866268357263371/9IV5Vbu-Zl71m1Xr46ciFny_ZUT_v1WA3KhM1j2MEFb9WiODu149B-VjCJeYHZHQ49xL"
...
function src.checkPolice()
	...
	if user_id then
		...
		elseif (os.time()-timedown) <= 1000 then
			...
			return false
		elseif vRP.getInventoryItemAmount(user_id, "lockpick") < 1 then
			TriggerClientEvent("Notify",source,"aviso","Você precisa de uma <b>Lockpick</b> na sua mochila", 8000)	
			return false
		end
	end
	return true

...
-- VARIABLES
-----------------------------------------------------------------------------------------------------------------------------------------
local inLocate = { 19.41,-1602.71,29.38 }
		...
		while true do
			if inService and inTimers > 0 then
				...
				if inTimers <= 0 then
					...
					local date = vSERVER.retornarData()
					TriggerEvent("NotifyPush",{ time = date, text = "Ola gostaria de estar comprando este produto.", code = 20, title = "Quero comprar um produto", x = pedLocate[rand][1], y = pedLocate[rand][2], z = pedLocate[rand][3], name = callName[math.random(#callName)].." "..callName2[math.random(#callName2)], rgba = {69,115,41} })
				end
			end
